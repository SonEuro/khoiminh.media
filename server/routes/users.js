...
router.put('/:id', (req, res) => {
  const { username, full_name, position, role, is_active, password, is_phan_lich, is_phan_lich_all, is_tra_ncc, is_quan_ly_kho, zalo_uid } = req.body;
  ...
  if (password) {
    db.prepare('UPDATE users SET username=?, full_name=?, position=?, role=?, is_active=?, is_phan_lich=?, is_phan_lich_all=?, is_tra_ncc=?, is_quan_ly_kho=?, zalo_uid=?, password_hash=? WHERE id=?')
      .run(username, full_name, position || '', role, is_active ? 1 : 0, pl, pla, tncc, qlk, zalo, bcrypt.hashSync(password, 10), id);
  } else {
    db.prepare('UPDATE users SET username=?, full_name=?, position=?, role=?, is_active=?, is_phan_lich=?, is_phan_lich_all=?, is_tra_ncc=?, is_quan_ly_kho=?, zalo_uid=? WHERE id=?')
      .run(username, full_name, position || '', role, is_active ? 1 : 0, pl, pla, tncc, qlk, zalo, id);
  }
  res.json({ ok: true });
});
