...
export default function MultiDatePicker({ value = [], onChange, error = false, placeholder = 'Chọn ngày...' }) {
    ...
    <>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'10px' }}>
        <button type="button" onClick={prevMonth}
          style={{ background:'rgba(167,139,250,0.12)', border:'none', color:'#a78bfa', cursor:'pointer', borderRadius:'6px', width:'32px', height:'32px', fontSize:'1.1rem', display:'flex', alignItems:'center', justifyContent:'center' }}>‹</button>
        <span style={{ fontWeight:700, color:'#e0e0ee', fontSize:'0.92rem' }}>
          ...
          return (
            <button key={i} type="button" onClick={() => toggle(d)}
              style={{
                ...
                border: isToday && !sel ? '1px solid rgba(167,139,250,0.5)' : 'none',
                cursor:'pointer', fontWeight: sel ? 800 : 400, fontSize:'0.85rem',
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'10px' }}>
        ...
        <button type="button" onClick={() => setOpen(false)}
          style={{ fontSize:'0.72rem', color:'#4ade80', background:'rgba(74,222,128,0.1)', border:'1px solid rgba(74,222,128,0.3)', borderRadius:'6px', padding:'4px 16px', cursor:'pointer', fontWeight:700 }}>
          Xong ✓
        </button>
      </div>
    </>
  ...
  return (
    <div ref={wrapperRef} style={{ position:'relative' }}>
      ...
      {open && isMobile && (
        /* Mobile: inline panel (cuộn theo form, không bị fixed clipping) */
        <div style={{
          marginTop:'6px',
