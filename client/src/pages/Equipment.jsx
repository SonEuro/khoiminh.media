...
export default function Equipment() {
  ...
  const catSummary = visibleCats.map(cat => {
    ...
    return {
      ...
      reserved:    items.reduce((s, e) => s + ((reservedEvents[e.id] || []).reduce((sum, r) => sum + r.qty, 0)), 0),
    };
  }).filter(c => c.total > 0);
    ...
    <div className="p-6">
      ...
      {topData && (
        <div style={{ marginBottom: '22px' }}>
          {/* Global stats */}
          <div className="grid grid-cols-2 lg:grid-cols-5" style={{ gap: '10px', marginBottom: '18px' }}>
            {[
                  ...
                  <div style={{ padding: '12px 14px', background: '#13131d', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    {(() => {
                      const stats = [
                        ...
                        { key: 'reserved',    label: 'Đặt trước', value: cat.reserved,    color: '#fb923c', bg: 'rgba(251,146,60,0.08)',  filterFn: e => (reservedEvents[e.id]?.length || 0) > 0,           qtyFn: e => (reservedEvents[e.id] || []).reduce((s,r) => s+r.qty, 0) },
