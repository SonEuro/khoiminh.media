      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                  ...
                  {extNoteOpen && (
                    <div style={{ borderTop:'1px solid rgba(96,165,250,0.12)', paddingTop:'8px', display:'flex', gap:'6px' }}>
                      ...
                      <input placeholder="ĐVT" value={it.unit} onChange={e => updateExtItem(idx, 'unit', e.target.value)}
                        style={{ width:'70px', flexShrink:0, padding:'6px 8px', borderRadius:'7px', border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.05)', color:'#e0e0ee', fontSize:'0.84rem', boxSizing:'border-box' }} />
                    </div>
                  )}
                </div>
              );
            ...
            <button onClick={addExtItem}
              style={{ width:'100%', padding:'9px', borderRadius:'8px', border:'1px solid rgba(96,165,250,0.7)', background:'rgba(96,165,250,0.18)', color:'#93c5fd', fontSize:'0.84rem', fontWeight:700, cursor:'pointer' }}>
              + Thêm nhà cung cấp
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              return (
                <div key={idx} style={{ padding:'8px 10px', borderRadius:'8px', background:'rgba(201,168,76,0.05)', border:'1px solid rgba(201,168,76,0.15)' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', gap:'8px' }}>
                    ...
                    {/* Phải: 2 flex rows, tất cả ô 46px */}
                    <div style={{ display:'flex', flexDirection:'column', gap:'5px', flexShrink:0 }}>
                      {/* Hàng 1: SL | FREE | combo# khi active */}
                      <div style={{ display:'flex', gap:'5px' }}>
                        <input type="number" min="1" value={it.quantity}
            ...
            <button type="button" onClick={addNewRow}
              style={{ width:'100%', padding:'9px', borderRadius:'8px', border:'1px solid rgba(201,168,76,0.7)', background:'rgba(201,168,76,0.18)', color:'#e8c97a', fontSize:'0.84rem', fontWeight:700, cursor:'pointer' }}>
              + Thêm thiết bị kho
            </button>
          </div>
        </div>
...
function ArchivedEventDetail({ ev, onSelect, onSelectReport }) {
  ...
  const txRow = (tx) => (
    <div key={tx.id} onClick={() => onSelect && onSelect(tx.id)} style={{ padding:'6px 10px', borderRadius:'6px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(120,120,160,0.1)', marginBottom:'3px', cursor: onSelect ? 'pointer' : 'default' }}>
...
function ArchivedEventRows({ events, isSuperAdmin, onUnarchive, onDelete, onSelect, onSelectReport }) {
  ...
  const monthGroups = (() => {
    ...
    for (const ev of filtered) {
      const key = tryKey(ev.archived_at) || tryKey(ev.start_date) || tryKey(ev.filming_date) || tryKey(ev.setup_date) || tryKey(ev.created_at) || '0000-00';
      if (!map[key]) {
        const [y, m] = key.split('-');
...
function TxRowsGrouped({ txs, onSelect, onDelete, onTraNcc, onTransfer, canPrint, outstandingExtMap = {}, khoOutstandingMap = {}, showNccBadge = false, nccOnlyEventSet = null }) {
  if (!txs.length) return <Empty text="Chưa có phiếu nào" />;
...
export default function Transactions() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      api.getTransactions({ type: 'TRA_NCC',                  limit: 50,  hide_archived: 'true' }),
      api.getEventReports(),
      api.getViolations(),
      api.getDashboard(),
